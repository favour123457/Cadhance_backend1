                ...
                <div class="widget-content widget-content-area">
                    ...
                    <form action="{{ route('countries.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code') }}">
                            @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Flag (emoji or URL)</label>
                            <input type="text" name="flag" class="form-control @error('flag') is-invalid @enderror" value="{{ old('flag') }}">
                            @error('flag')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Continent</label>
                            <input type="text" name="continent" class="form-control @error('continent') is-invalid @enderror" value="{{ old('continent') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="sanctioned" class="form-select @error('sanctioned') is-invalid @enderror">
                                <option value="0" {{ old('sanctioned', '0') == '0' ? 'selected' : '' }}>No</option>
                                <option value="1" {{ old('sanctioned') == '1' ? 'selected' : '' }}>Yes</option>
                            </select>
