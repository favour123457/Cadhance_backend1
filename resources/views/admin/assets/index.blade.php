                    ...
                    <div class="table-responsive">
                        <table id="html5-extension" class="table table-hover" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $asset->is_pinned ? 'table-warning' : '' }}">
                                    <td>
                                        @if($asset->is_pinned)
                                            <span class="badge badge-warning" title="Pinned to position #{{ $asset->pin_position }}">
                                                &#9733; #{{ $asset->pin_position }}
                                            </span>
                                        @else
                                            {{ $i + 1 }}
                                    ...
                                    <td>{{ $asset->design_category->name ?? 'N/A' }}</td>
                                    <td><span class="badge badge-light-info">{{ $asset->asset_status->name ?? 'N/A' }}</span></td>
                                    <td>{{ showMoney($asset->price ?? 0) }}</td>
                                    <td>{{ $asset->created_at->format('M d, Y') }}</td>
                                    <td>
                                        {{-- Pin / Unpin (Top Priority) button --}}
                                        <form action="{{ route('assets.toggle-pin', $asset->id) }}" method="POST" class="d-inline"
                                              onsubmit="return confirm('{{ $asset->is_pinned ? 'Remove this asset from top position?' : 'Pin this asset to a top position? (max 2 allowed)' }}');">
                                            @csrf
                                            <button type="submit"
                                                class="btn btn-sm {{ $asset->is_pinned ? 'btn-warning' : 'btn-outline-warning' }}"
                                                title="{{ $asset->is_pinned ? 'Unpin from top' : 'Pin to top (Priority)' }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="{{ $asset->is_pinned ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
                                                {{ $asset->is_pinned ? 'Unpin' : 'Top' }}
                                            </button>
                                        </form>

                                        @if(checkButtonPermission('assets', 'show'))
                                        ...
                                        @endif
                                        @if(checkButtonPermission('assets', 'delete'))
                                        <form action="{{ route('assets.destroy', $asset->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?');">
                                            @csrf @method('DELETE')
