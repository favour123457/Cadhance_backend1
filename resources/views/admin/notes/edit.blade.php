                    ...
                    <div class="widget-content widget-content-area">
                        ...
                        <form action="{{ route('notes.update', $note->id) }}" method="POST">
                                ...
                                <select name="note_type_id" class="form-select @error('note_type_id') is-invalid @enderror"
                                    ...
                                    @foreach ($noteTypes as $type)
                                        <option value="{{ $type->id }}"
                                            {{ old('note_type_id', $note->note_type_id) == $type->id ? 'selected' : '' }}>
                                            {{ $type->name }}</option>
                                    @endforeach
                                </select>
                                @error('note_type_id')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="note" rows="5" class="form-control @error('note') is-invalid @enderror" id="myeditorinstance" required>{{ old('note', $note->note) }}</textarea>
                                @error('note')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
